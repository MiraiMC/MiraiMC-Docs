...
某些情况下，使用[扫码登录](../tutorial/use-qrcode-login.md)可能可以解决 `登录失败 code=45/235/237/238` 的问题。
...
如果你无论如何都无法正常登录，但可以登录官方 QQ 程序，并且你的服务端运行于完整的服务器环境中（即可以 SSH 连接的 Linux 系统或远程桌面连接的 Windows 系统，面板服务商不在此列），那么可以尝试 [MiraiMC 的 Overflow 插件](https://github.com/DreamVoid/MiraiMCAddon-Overflow)。
...
## 在 Bukkit+Forge 融合服务端使用时出现报错、无法正常加载插件
...
但是，mirai 乃至整个机器人生态都在不断变化，有时你可能会需要 [MiraiMC 的 Overflow 插件](https://github.com/DreamVoid/MiraiMCAddon-Overflow)或第三方签名服务来满足你的需求，这种情况时有发生，因此最好做好使用外部程序的准备。
